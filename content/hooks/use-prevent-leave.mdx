   ...
   <div className="fd-steps">
    <div className="fd-step" >
      <p>다음 코드를 프로젝트에 복사/붙혀넣기 하세요.</p>
        <ComponentSource src="registry/default/hooks/usePreventLeave.ts" title="hooks/usePreventLeave.ts" />
    </div>
   </div>
  </Tab>
</Tabs>
...
| 속성      | 타입      | 기본값 | 설명                                                          |
| --------- | --------- | ------ | ------------------------------------------------------------- |
| `prevent` | `boolean` | `true` | 이 값이 `true`일 때 페이지 이탈(새로고침/닫기 등)을 방지합니다. |
